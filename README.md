...
In this lab we expand on our knowledge of sorting data by filtering,
selecting, and arranging data into tables. With these tables we can then
compare data sets using different graphs. New skills such as creating
and renaming variables, summarizing datasets, and visualizing variable
relationships.
...
> 2.  Convert the four numerical variables from mm to cm by multiplying
...
>     maximum value minimum value range median first quartile (q1) third
>     quartile (q2) inter-quartile range (iqr) Calculate the following
>     statistics for each species from the petal width variable and
...
```
    ...
    ## # A tibble: 1 x 8
    ##   sample_size   max   min range median first_quartile third_quartile   IQR
...
>     standard deviation variance standard error of the mean approximate
...
```
    ...
    ## # A tibble: 3 x 8
    ##   species    sample_size  mean    sd   var standard_error conf_upper conf_lower
...
> 6 and 7. Visualize the relationship between petal length and species
...
>     species using a scatterplot. Map the two numerical variables to
>     the x and y axes and map species to the color and shape
>     aesthetics.
